...
$InstType = (get-ec2instance).RunningInstance | Where-Object { $_.InstanceId -eq $Instance.ResourceId[0] }
...
$BucketName = "vizrtmeasurelogs-"+$Type
if (Test-S3Bucket -BucketName ($BucketName))
{
    Write-Host "Bucket is there!"
}else{
    New-S3Bucket -BucketName $BucketName -Region eu-central-1 }
Write-S3Object -BucketName $BucketName -File '.\g4dn.xlarge-031122.091237-EC2AMAZ-JOIN (EC2AMAZ-SOURCE (Bild 1080p30)).csv'