...
#Query for the running processes and starting them
$session = New-PSSession -ComputerName 10.0.1.20 -ErrorAction Stop
Invoke-Command -Session $session -ScriptBlock {
if(!(Get-Process | Where-Object { $_.ProcessName -eq "NDI Discovery Service" }))
{
...
{
    Start-process "C:\Program Files\NDI\NDI 5 Tools\Bridge\Application.NDI.VPN.UI.exe" /host
}
